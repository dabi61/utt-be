...
class QRCodeAttendanceSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if now < schedule.start_time - timedelta(minutes=5):
            raise serializers.ValidationError(f"Chưa đến thời gian điểm danh. Bạn có thể điểm danh 5 phút trước giờ học bắt đầu ({timezone.localtime(schedule.start_time).strftime('%H:%M:%S %d/%m/%Y')})")
